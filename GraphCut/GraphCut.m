...
%             When using SparseSmoothness Dc is of (L)x(P) where L is the %% Msun: use this format to handle superpixel
...
%           - SparseSmoothness: a sparse matrix defining both the graph %% Msun: use this format to handle superpixel
...
%	<!--Msun
%   - 'open': Create a new graph object
%           [gch] = GraphCut('open_hop', DataCost, SmoothnessCost, SparseSmoothness, hop);
%
%       Inputs:
%           - DataCost a height by width by num_labels matrix where
%             Dc(r,c,l) equals the cost for assigning label l to pixel at (r,c)
...
%
%   - 'expand_qpbo': Perform labels expansion, solving using QPBO
%           [gch labels] = GraphCut('expand_QPBO', gch)
%           [gch labels] = GraphCut('expand_QPBO', gch, iter)
%           [gch labels] = GraphCut('expand_QPBO', gch, [], label)
%           [gch labels] = GraphCut('expand_QPBO', gch, [], label, indices)
%
%       When no inputs are provided, GraphCut performs expansion steps
%       until it converges.
%
...
switch lower(mode)
    ...
    case {'open_hop'}
        % open a new graph cut
        if nargout ~= 1
            error('GraphCut:Open_hop: wrong number of output arguments');
        end

        gch = OpenGraphHOP(varargin{:});
    ...
    case {'y', 'yset'}
        % set user defined labeling
        if nargout ~= 1
            error('GraphCut:SetDetLabels: Too many outputs');
        end
        
        [gch] = SetDetLabels(varargin{:});
        
    case {'f'}
        % set which detection indicator should be fixed
        if nargout ~= 1
            error('GraphCut:SetDetFixBook: Too many outputs');
        end
        [gch] = SetDetFixBool(varargin{:});
        
    case {'dcen', 'dcn', 'det_class_energy'}
        % get current energy values
        if nargin < 2
            error('GraphCut:GetEnergy: too many input arguments');
        end
        gch = varargin{1};
        det_class = int32(varargin{2});
        [gch dce] = GraphCutMex(gch, 'd', det_class);
        varargout{1} = dce;        
    case {'det_class_energy_xy'}
        % get current energy values
        if nargin < 2
            error('GraphCut:GetEnergy: too many input arguments');
        end
        gch = varargin{1};
        det_class = int32(varargin{2});
        [gch dce] = GraphCutMex(gch, 'x', det_class);
        varargout{1} = dce;   
    case {'det_class_energy_prior'}
        % get current energy values
        if nargin < 2
            error('GraphCut:GetEnergy: too many input arguments');
        end
        gch = varargin{1};
        det_class = int32(varargin{2});
        [gch dce] = GraphCutMex(gch, 'p', det_class);
        varargout{1} = dce;   
    ...
    case {'en', 'n', 'energy'}
        ...
        gch = varargin{1};
        [gch se de he ye oe pte coe] = GraphCutMex(gch, 'n');
        switch nargout
            case 2
                varargout{1} = se+de;
            case 3
    ...
    case {'e', 'ex', 'expand'}
        ...
        [gch labels1 labels2] = Expand(varargin{:});
        if nargout == 2
            varargout{1} = labels1;
        elseif nargout == 3
            varargout{1} = labels1;
...
%<!--Msun
    case {'expand_qpbo'}
        % use expand steps to minimize energy
        if nargout > 2
            error('GraphCut:Expand_QPBO: too many output arguments');
        end
        [gch labels] = Expand_QPBO(varargin{:});
        if nargout == 2
            varargout{1} = labels;
        end

    case {'expand_qpbo_det'}
        % use expand steps to minimize energy
        if nargout > 2
            error('GraphCut:Expand_QPBO_DET: too many output arguments');
        end
        [gch labels] = Expand_QPBO_DET(varargin{:});
        if nargout == 2
            varargout{1} = labels;
        end
% Msun -->
    ...
    case {'sw', 'a', 'ab', 'swap'}
        ...
        if nargout > 2
            error('GraphCut:Swap: too many output arguments');
        end
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function gch = OpenGraphHOP(varargin)
% or    [gch] = OpenGraphHOP('open', DataCost, SmoothnessCost, SparseSmoothness, hop) - any graph
% Data cost
Dc = varargin{1};
...
if length(varargin) > 6
    pair_thing = varargin{7};
    if isfield(pair_thing, 'spatial'),
        pair_thing = rmfield(pair_thing, 'spatial');
    end
    if isempty(pair_thing),
        pair_thing = struct([]);
    end
...
if length(varargin) > 8
    class_co_uw = varargin{8};
    class_co_pw = varargin{9};
else
    class_co_uw = [];
    class_co_pw = [];
end
if length(varargin) > 9
   comp_label = varargin{10};
else
   comp_label = struct([]);
end
gch = GraphCutConstrSparseHOP(single(Dc(:)), single(Sc(:)), SparseSc, hop, hop_det, hop_occ, pair_thing, single(class_co_uw), single(class_co_pw), comp_label);
...
elseif ndims(Dc) == 2
    ...
    gch = GraphCutConstrSparse(single(Dc(:)), single(Sc(:)), SparseSc); % Msun: use this to contruct arbitrary graph
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [gch] = SetDetLabels(varargin)
% usgae [gch] = SetDetLabels(gch, labels)

if nargin ~= 2
    error('GraphCut:SetDetLabels: wrong number of inputs');
end
gch = varargin{1};
labels = varargin{2};

if ~strcmp(class(labels), 'logical')
    labels = logical(labels);
end
labels = labels';
[gch] = GraphCutMex(gch, 'y', labels(:));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [gch] = SetDetFixBool(varargin)
% usgae [gch] = SetDetFixBool(gch, FixBool)

if nargin ~= 2
    error('GraphCut:SetDetFixBool: wrong number of inputs');
end
gch = varargin{1};
FixBool = varargin{2};

if ~strcmp(class(FixBool), 'logical')
    FixBool = logical(FixBool);
end
FixBool = FixBool';
[gch] = GraphCutMex(gch, 'f', FixBool(:));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [gch labels1 labels2] = Expand(varargin)
gch = varargin{1};
switch nargin
    case 1
        [gch labels1 labels2] = GraphCutMex(gch, 'e');
    case 2
        [gch labels1 labels2] = GraphCutMex(gch, 'e', varargin{2});
    case 3
        [gch labels1 labels2] = GraphCutMex(gch, 'e', varargin{2}, varargin{3});
    case 4
        ind = varargin{4};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%// <!--Msun
function [gch labels] = Expand_QPBO(varargin)
gch = varargin{1};
switch nargin
    case 1
        [gch labels] = GraphCutMex(gch, 'q');
    case 2
        [gch labels] = GraphCutMex(gch, 'q', varargin{2});
    case 3
        [gch labels] = GraphCutMex(gch, 'q', varargin{2}, varargin{3});
    case 4
        ind = varargin{4};
        ind = int32(ind(:)-1)'; % convert to int32
        [gch labels] = GraphCutMex(gch, 'q', varargin{2}, varargin{3}, ind);
    otherwise
        error('GraphCut:Expand: wrong number of inputs');
end
labels = labels';

function [gch labels] = Expand_QPBO_DET(varargin)
gch = varargin{1};
switch nargin
    case 1
        [gch labels] = GraphCutMex(gch, 'd');
    case 2
        [gch labels] = GraphCutMex(gch, 'd', varargin{2});
    case 3
        [gch labels] = GraphCutMex(gch, 'd', varargin{2}, varargin{3});
    case 4
        ind = varargin{4};
        ind = int32(ind(:)-1)'; % convert to int32
        [gch labels] = GraphCutMex(gch, 'd', varargin{2}, varargin{3}, ind);
    otherwise
        error('GraphCut:Expand: wrong number of inputs');
end
labels = labels';
